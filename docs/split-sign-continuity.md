...
## Funding operator preflight (September 26, 2026)

`script/generate_merchant_funding_agreement.rb` now validates its configuration
through `lib/funding_deal_terms.rb` before accessing templates. The operator
must supply explicit `purchase_price`, `purchased_amount`, `factor_rate`,
`origination_fee`, `prior_balance`, `ach_program_fee`, `net_amount_funded`,
`specified_percentage`, `agreement_date` (YYYY-MM-DD),
`initial_periodic_amount`, and `terms_reference`, in addition to `merchant_id`.
The reference identifies the reviewed provider/deal source; merely supplying a
reference does not constitute approval. Currency inputs require finite,
nonnegative whole-cent values, and purchased amount must match the supplied
price/factor. Net proceeds are supplied, not estimated from an incomplete fee
list. A zero fee or a variable periodic amount must also be explicit.

An existing combined funding template with any submission is preserved and
cannot be regenerated by this helper. Use the issued packet's existing review
link. Changing terms or issuing a replacement requires its separately approved
workflow; the helper does not rewrite the issued packet or its historical
snapshot.

The current packet's disclosure-page guard remains closed for FL, UT, NY, CA,
VA, and CT. Completing it requires approved form versions and selection policy,
provider-approved financial definitions and deal inputs, and reviewed field and
signature mappings. No APR, funding/disclosure date, forecast term, repayment
schedule, or legal applicability is inferred by the new preflight. Passing its
unit checks is not certification that a funding packet is ready for issuance.

Run `script/test_funding_deal_terms.rb` and
`script/test_funding_packet_layout.rb` with the application's bundled Ruby.
The standalone system Ruby may lack BigDecimal. Operator-only releases may
update the entrypoint and helper without rebuilding the application image,
provided the previous files are backed up privately and source/runtime hashes
are verified. No merchant generation or submission is needed for these tests.
